...
/**
  * @brief 	Set the cursor at coordinates (row, col).
...
/**
  * @brief 	Print the box id on LCD.
  * @param 	bID: Box id to print.
  * @retval None
  */
void Lcd_Show_Box_ID(uint16_t bID){

	char strBoxID[16];
	sprintf((char*)strBoxID, "Box ID: %d", bID);
...
uint8_t PCF_Scan(){
	...
	UART_DEBUG_Transmit((uint8_t*)"Scan I2C");
	for( i = 1; i< 128; i++){
		result = HAL_I2C_IsDeviceReady(&I2cHandle, (uint16_t) (i<<1), 2, 2);
		if(result != HAL_OK){
			UART_DEBUG_Transmit((uint8_t*)"."); // No ACK received at that address
		}
		else{
			count ++;
