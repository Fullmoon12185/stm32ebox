...
static void MQTT_on_connect_cb(uint8_t status);
static void MQTT_on_disconnect_cb(uint8_t status);
static void MQTT_on_message_cb(char * topic, char * payload);
static void MQTT_on_publish_cb(uint8_t status);
...
static netif_mqtt_client_t mqtt_client = {
		...
		.reconnect = 1,
		.keep_alive = 120,
		.on_connect = MQTT_on_connect_cb,
		.on_disconnect = MQTT_on_disconnect_cb,
		.on_message = MQTT_on_message_cb,
		.on_publish = MQTT_on_publish_cb
};
